...
export type ScanResult = { blocked:boolean; risk:number; sanitized_text:string; detectors:{heuristic:DetectorResult;llm:DetectorResult;probe:DetectorResult};findings:Finding[];policy:{id:string;name:string;version:number;threshold:number;reason:string};provenance?:{provider:'openai'|'openrouter'|'aws';model:string;source:'workspace'|'environment'|'aws';fallback:boolean;signal:'semantic'|'inferred-probe'|'activation-probe'}[];degraded?:boolean }
