...
async function resolveRemoteDetectors(text:string,source:TrustLevel,ownerId:string){
 ...
 if(process.env.OPENAI_SECRET_ID&&process.env.PROBE_TOKEN_SECRET_ID&&process.env.PROBE_SERVICE_URL){
  try{const [llm,probe]=await Promise.all([withTimeout(awsLlmJudge(text,source),12000,'AWS LLM judge'),withTimeout(activationProbe(text),15000,'Activation probe')]);trace.push({provider:'aws',model:process.env.OPENAI_MODEL||'gpt-5.6',source:'aws',fallback:false,signal:'semantic'},{provider:'aws',model:process.env.PROBE_MODEL||'deberta-v3-base-prompt-injection-v2',source:'aws',fallback:false,signal:'inferred-probe'});return{llm,probe,provenance:trace,errors}}catch(error){errors.push(error instanceof Error?error.message:'AWS pipeline failed')}
 }
 throw new Error(`No complete detector path is available. Configure OPENAI_API_KEY or OPENROUTER_API_KEY. ${errors.join(' ')}`)
}
